...
def _survey_time_seconds(survey_years, include_kludge=True):
    """
    Effective survey time in seconds, matching the SO_fisher/noisecalc.py convention:
      t = survey_years * (365.25 days) * 86400 * (0.2 * 0.85)
    where 0.2 is the observing efficiency/retention and 0.85 is the map-edge
    non-uniformity kludge used in the public V3 calculators.
    """
    t = float(survey_years) * 365.25 * 24.0 * 3600.0
    t *= 0.2
    if include_kludge:
        t *= 0.85
    return t
...
def Simons_Observatory_V3_LA_noise(
    ...
    *args,
    ...
    #   - legacy: Simons_Observatory_V3_LA_noise(sensitivity_mode, f_sky, ell_max, delta_ell, ...)
    #   - preferred: Simons_Observatory_V3_LA_noise(sensitivity_mode, survey_years, f_sky, ell_max, delta_ell, ...)
    # sensitivity_mode:
    #     1: baseline, 
    #     2: goal
    # survey_years: total calendar years
    ...
    if len(args) == 3:
        survey_years = 5.0
        f_sky, ell_max, delta_ell = args
    elif len(args) >= 4:
        survey_years, f_sky, ell_max, delta_ell = args[:4]
    else:
        raise TypeError(
            "Expected (f_sky, ell_max, delta_ell) or (survey_years, f_sky, ell_max, delta_ell)."
        )
    ####################################################################
    ####################################################################
    ###                        Internal variables
    ...
    t = _survey_time_seconds(survey_years, include_kludge=True)
...
def Simons_Observatory_V3_SA_noise(sensitivity_mode,one_over_f_mode,SAT_yrs_LF,f_sky,ell_max,delta_ell,
    ...
    # allow arbitrary survey duration by rescaling noise NEW
    SAT_yrs_input = SAT_yrs_LF
    SAT_yrs_LF = min(SAT_yrs_LF, 5)
    assert( f_sky > 0. and f_sky <= 1.)
    assert( ell_max <= 2e4 )
    assert( delta_ell >= 1 )
    # configuration
    ...
    ####################################################################
    ## calculate the survey area and time
    # Historically the public V3 calculator assumed a 5-year survey.
    # Here we allow arbitrary survey duration by rescaling the effective
    # integration time, while still using the LF/MF tube-mix logic above
    # capped at 5 years for config purposes.
    t = _survey_time_seconds(SAT_yrs_input, include_kludge=include_kludge)
    ...
    if beam_stuff:
        ...
        B_ell = Simons_Observatory_V3_SA_beams(ell)
        N_ell_P_27  /= B_ell[0]**2
        N_ell_P_39  /= B_ell[1]**2
        N_ell_P_93  /= B_ell[2]**2
        N_ell_P_145 /= B_ell[3]**2
        N_ell_P_225 /= B_ell[4]**2
        N_ell_P_280 /= B_ell[5]**2
